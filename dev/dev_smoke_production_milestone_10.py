...
OUT_ROOT = REPO_ROOT / "dev" / "_smoke_artifacts" / "production_milestone_10"
SELF_STATUS_PATHS = {
...
def _assert_polished_html(html: str) -> None:
    ...
    assert "Controlled Failure Detail" in html, html
    assert "Expected Failure Type" in html, html
    assert "missing_selector" in html, html
    assert "Failed Action" in html, html
    assert "wait_for_selector" in html, html
    assert "selector_ref" in html, html
    assert "pm8.missing_element" in html, html
    assert "Resolved Selector" in html, html
    assert "#pm8-never-appears" in html, html
    assert "Error Class" in html, html
    assert "Error Message" in html, html
    assert "Controlled expected failure" in html, html
    assert "<pre>" not in html, html
    assert "Result / Evidence" in html, html
    assert "Run Duration" in html, html
    assert "Run Operations Summary" in html, html
    assert "Manifest Records Total" in html, html
    assert "Retry Attempts" in html, html
    assert "Continued After Failure" in html, html
    assert "Agent/Worker Count" in html, html
    assert "Per-Agent Attempted" in html, html
    assert html.count("Not captured yet") >= 8, html
    assert "Not applicable for this fixture-bound proof" in html, html
...
def dev_smoke() -> Path | None:
    ...
    _assert_polished_html(output_path.read_text(encoding="utf-8"))
...
if __name__ == "__main__":
    try:
        ...
        print(f"SKIP: production_milestone_10 real browser unavailable: {exc}")
    else:
        print(f"PROOF_DEMO: {demo_path}")
        print("DEV_SMOKE_OK: production_milestone_10")
