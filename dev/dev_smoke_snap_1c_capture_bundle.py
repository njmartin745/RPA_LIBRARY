from __future__ import annotations  
  
import sys   
from pathlib import Path  
  
PROJECT_ROOT = Path(__file__).resolve().parents[1]  
if str(PROJECT_ROOT) not in sys.path:  
    sys.path.insert(0, str(PROJECT_ROOT))  
  
from SNAP.snap_1a_workflow_capture import CapturedEvent  
from SNAP.snap_1c_capture_bundle import (  
    build_capture_bundle_from_events,  
    validate_capture_bundle,  
)  
  
__all__ = ["dev_smoke"]  
  
  
def _repo_root() -> Path:  
    return Path(__file__).resolve().parent.parent  
  
  
def dev_smoke() -> None:  
    events = [  
        CapturedEvent(kind="click", seq=1, selector="#login"),  
        CapturedEvent(kind="navigate", seq=2, url="https://example.test/app"),  
        # changes excluded by default to avoid value capture in the bundle at this stage  
        CapturedEvent(kind="change", seq=3, selector="input[name=\"username\"]", value="alice"),  
    ]  
  
    bundle = build_capture_bundle_from_events(  
        events,  
        bundle_name="captured",  
        selector_pack_name="captured",  
        selector_ref_prefix="cap",  
        include_clicks=True,  
        include_navigation=True,  
        include_changes=False,  # keep conservative for capture bundle  
    )  
  
    # Validate against registry + supported action set  
    validate_capture_bundle(bundle, repo_root=_repo_root(), require_registry_compat=True)  
  
    # Ensure selector_ref-first happened for click (since selector pack includes it)  
    steps = bundle["workflow"]["steps"]  
    assert steps[0]["action"] == "click_selector"  
    assert "selector_ref" in steps[0]  
    assert "selector" not in steps[0]  
  
    print("PASS")  
  
  
if __name__ == "__main__":  
    dev_smoke()  