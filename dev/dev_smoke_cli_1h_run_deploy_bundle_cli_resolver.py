...
from CLI.cli_1h_run_deploy_bundle_cli_resolver import (  
    main,  
...
def dev_smoke() -> None:  
    ...
    assert mod.__name__.endswith("cli_1g_run_deploy_bundle_with_report_fail_fast")  
    ...
    # Make report invalid (missing version) but still runnable under --no-validate.  
    dep = dict(dep)  
    if "version" in dep:  
        del dep["version"]  
